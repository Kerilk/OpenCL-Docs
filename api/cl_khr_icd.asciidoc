...
Accepted as terminator to the _properties_ list of
{clCreateInstanceKHR}.
...
New in version 2.0.1, the loader now supports deinitialization and platform
that support unloading their driver can advertise it through the
{cl_khr_icd_unloadable_EXT} extension.
--

  . How will the ICD Loader that support deinitialization enable compatibility
    with old layers.

+
--
The ICD Loader that support deinitialization will check for the
`OCL_ICD_FORCE_LEGACY_TERMINATION` environment variable, and revert to the
legacy behavior if it evaluates to true.
--
